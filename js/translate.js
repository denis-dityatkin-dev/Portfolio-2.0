...
const translations = {
	...
	uk: {
		...
		factWorkStudy: 'Я поєдную роботу та навчання із захопленням веб-розробкою',
...
// Простой textContent: селектор → ключ перевода
const TEXT_MAP = {
	// HEADER
	'.menu__item:nth-child(1) [data-i18n="nav.home"]': 'navHome',
	'.menu__item:nth-child(2) [data-i18n="nav.works"]': 'navWorks',
	'.menu__item:nth-child(3) [data-i18n="nav.about"]': 'navAbout',
	'.menu__item:nth-child(4) [data-i18n="nav.contacts"]': 'navContacts',

	// HERO
	'.main-section__text p': 'heroText',
	'.main-section__button': 'heroButton',

	// WHO
	'.who__text p': 'whoText',
	'.who__author': 'whoAuthor',

	// PROJECTS HOME (кнопка "View all")
	'.projects__link': 'viewAll',

	// ABOUT HOME
	'.about__button': 'readMore',

	// CONTACTS HOME
	'.contacts__text p': 'contactsText',
	'.contacts__subtitle': 'messageMe',

	// ABOUT PAGE
	'.about-me__header .sub-title': 'aboutPageSubtitle',
	'[data-i18n="aboutPageHello"]': 'aboutPageHello',
	'[data-i18n="aboutPageText1"]': 'aboutPageText1',
	'[data-i18n="aboutPageText2"]': 'aboutPageText2',

	// CONTACTS PAGE
	'.contact__header .sub-title': 'contactsPageSubtitle',
	'.contact-section__text p': 'contactsPageText',
	'.contact-section__content:nth-child(1) .contacts__title': 'contactMessageTitle',
	'.contact-section__content:nth-child(2) .contacts__title': 'contactAvailabilityTitle',
	'.contact-section__content:nth-child(2) .contacts__item': 'contactAvailability',

	// PRICING
	'.pricing__header .sub-title': 'servicesSubtitle',
	'.pricing__note p': 'pricingNote',

	// WHAT I DO
	'.services__header .sub-title': 'whatIDoSubtitle',

	// HOW I WORK
	'.process__header .sub-title': 'howIWorkSubtitle',

	// FINAL CTA
	'.contact-cta__content p': 'finalCtaText',
	'.contact-cta__button': 'getInTouch',

	// FOOTER
	'.body-footer__text p': 'footerText',
	'.media-footer__title': 'media',
...
const HASH_TITLES = {
	...
	'.skills__title': 'skillsTitle',
	'.about__title': 'aboutTitle',
...
const PROJECTS_HOME = ['arct', 'arik', 'gronberg'];

const PROJECTS_WORKS = [
	'arct', 'arik', 'gronberg', 'weeb', 'saulDesign', 'minimalBlog',
	'freedom', 'debonaire', 'wildKingdom', 'growfy', 'homeServices', 'lidia'
];
...
// ==================== ФАКТЫ / СЕРВИСЫ / ПРОЦЕСС ====================

const FACTS = [
	'factAutumn', 'factCycling', 'factPuzzles', 'factLearning', 'factBasketball',
	'factPizza', 'factMovie', 'factHabits', 'factWorkStudy'
];

const SERVICES = [
	['responsiveDesignTitle', 'responsiveDesignText'],
	['figmaToHtmlTitle', 'figmaToHtmlText'],
	['cleanCodeTitle', 'cleanCodeText'],
	['animationsTitle', 'animationsText'],
	['crossBrowserTitle', 'crossBrowserText'],
	['gitGithubTitle', 'gitGithubText']
];

const PROCESS = [
	['discussionTitle', 'discussionText'],
	['developmentTitle', 'developmentText'],
	['testingTitle', 'testingText'],
...
function setLanguage(lang) {
	...
	// -------- Простые textContent --------
	Object.entries(TEXT_MAP).forEach(([sel, key]) => setText(sel, t[key]));

	// -------- Заголовки с # и / --------
	Object.entries(HASH_TITLES).forEach(([sel, key]) =>
		setHTML(sel, `<span>#</span>${t[key]}`)
	);
	Object.entries(SLASH_TITLES).forEach(([sel, key]) =>
		setHTML(sel, `<span>/</span>${t[key]}`)
	);

	// -------- Hero (со span внутри) --------
	setHTML('.main-section__title', `${t.heroTitle}<span>${t.heroDeveloper}</span>`);
	setHTML('.decor-main-section__subtitle', `${t.heroAvailable}<span>${t.heroFreelance}</span>`);

	// -------- About home: 3 абзаца --------
	const aboutKeys = ['aboutHello', 'aboutText1', 'aboutText2'];
	$$('.about__text p').forEach((p, i) => {
		if (aboutKeys[i]) p.textContent = t[aboutKeys[i]];
	});

	// -------- Проекты home --------
	PROJECTS_HOME.forEach((name, i) => {
		const n = i + 1;
		setText(`.projects__item:nth-child(${n}) .body-item-projects__link-title`, t[`${name}Title`]);
		setText(`.projects__item:nth-child(${n}) .body-item-projects__text p`, t[`${name}Text`]);
	});

	// -------- Проекты works --------
	PROJECTS_WORKS.forEach((name, i) => {
		const n = i + 1;
		setText(`.work .projects__item:nth-child(${n}) .body-item-projects__link-title`, t[`${name}WorkTitle`]);
		setText(`.work .projects__item:nth-child(${n}) .body-item-projects__text p`, t[`${name}WorkText`]);
	});

	// -------- Кнопки проектов (дома + works одним селектором) --------
	setTextAll('.projects__item .item-projects__button.button__live:first-child', t.liveDemo);
	...
	// -------- Fun facts --------
	FACTS.forEach((key, i) =>
		setText(`.facts .content-fact__item:nth-child(${i + 1})`, t[key])
	);

	// -------- Pricing --------
	PRICING_ITEMS.forEach((item, i) => {
		const n = i + 1;
		const base = `.pricing__item:nth-child(${n})`;

		setText(`${base} .pricing__title`, t[item.title]);
		setText(`${base} .pricing__text p`, t[item.text]);

		const timeEl = $$('.pricing__time')[i];
		if (timeEl) {
			timeEl.innerHTML = `<span>${t.estimatedTime}</span>${t[item.time]}`;
		}

		item.prices.forEach(([sizeKey, priceKey], j) => {
			const m = j + 1;
			setText(`${base} .pricing__price-item:nth-child(${m}) span:nth-child(1)`, t[sizeKey]);
			setText(`${base} .pricing__price-item:nth-child(${m}) span:nth-child(2)`, t[priceKey]);
		});
	});

	// -------- What I do --------
	SERVICES.forEach(([titleKey, textKey], i) => {
		const item = $$('.services__item')[i];
		if (!item) return;
		const h3 = item.querySelector('h3');
		const p = item.querySelector('p');
		if (h3) h3.textContent = t[titleKey];
		if (p) p.textContent = t[textKey];
	});

	// -------- How I work --------
	PROCESS.forEach(([titleKey, textKey], i) => {
		const item = $$('.process__item')[i];
		if (!item) return;
		const h3 = item.querySelector('h3');
		const p = item.querySelector('p');
		if (h3) h3.textContent = t[titleKey];
